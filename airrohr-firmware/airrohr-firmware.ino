...
#include <Arduino.h>
// increment on change
#define SOFTWARE_VERSION_STR "FEM-2024-ESP32-V1.0"
 ...
 *****************************************************************/
Bme68x bme68x;
const uint8_t bme68x_default_i2c_address = 0x77;
const uint8_t bme68x_alternate_i2c_address = 0x76;
...
float last_value_BME68x_T = -128.0;
float last_value_BME68x_P = -1.0;
float last_value_BME68x_H = -1.0;
...
#pragma GCC diagnostic pop
	...
	if (!err)
	{
		...
		{
            /* FEM version - hack: bmx280 flag da rimappare su bme68x_read*/
			//cfg::bmx280_read = true;
            cfg::bme68x_read = true;
			rewriteConfig = true;
		}
	}
	else
...
static void webserver_values()
{
	...
	if (cfg::ppd_read)
	{
		add_table_value(FPSTR(SENSORS_PPD42NS), FPSTR(WEB_PM10), check_display_value(last_value_PPD_P1, -1, 1, 0), FPSTR(INTL_PARTICLES_PER_LITER));
    ...
    //FEM Added display on web server of BME680
	if (cfg::bme68x_read)
	{        
		add_table_t_value(FPSTR(SENSORS_BME680), FPSTR(INTL_TEMPERATURE), last_value_BME68x_T);
		add_table_value(FPSTR(SENSORS_BME680), FPSTR(INTL_PRESSURE), check_display_value(last_value_BME68x_P / 100.0f, (-1 / 100.0f), 2, 0), unit_P);
		//add_table_value(FPSTR(SENSORS_BME680), FPSTR(INTL_PRESSURE_AT_SEALEVEL), last_value_BME68x_P != -1.0f ? String(pressure_at_sealevel(last_value_BMX280_T, last_value_BMX280_P / 100.0f), 2) : "-", unit_P);
        add_table_h_value(FPSTR(SENSORS_BME680), FPSTR(INTL_HUMIDITY), last_value_BME68x_H);
        dew_point_temp = dew_point(last_value_BME68x_T, last_value_BME68x_H);
        add_table_value(FPSTR(SENSORS_BME680), FPSTR(INTL_DEW_POINT), isnan(dew_point_temp) ? "-" : String(dew_point_temp, 1), unit_T);
		page_content += FPSTR(EMPTY_ROW);
	}    
	if (cfg::sht3x_read)
	{
...
/*****************************************************************
 * read BME68x sensor values                                     *
 *****************************************************************/
static void fetchSensorBME68x(String &s)
{
	bme68xData data;

    debug_outln_verbose(FPSTR(DBG_TXT_START_READING), FPSTR(SENSORS_BME680));
	...
	{
		bme68x.getData(data);
		const auto t = data.temperature;
		const auto p = data.pressure;
		const auto h = data.humidity;

		last_value_BME68x_T = t + readCorrectionOffset(cfg::temp_correction);
		last_value_BME68x_P = p;
		last_value_BME68x_H = h;

		//NOTE FEM: BME68x_* tags are NOT ACCEPTED, replacing as it were BME280...
		add_Value2Json(s, F("BME280_temperature"), FPSTR(DBG_TXT_TEMPERATURE), last_value_BME68x_T);
		add_Value2Json(s, F("BME280_pressure"), FPSTR(DBG_TXT_PRESSURE), last_value_BME68x_P);
		add_Value2Json(s, F("BME280_humidity"), FPSTR(DBG_TXT_HUMIDITY), last_value_BME68x_H);
	}
	else
	{
		last_value_BME68x_T = -128.0;
		last_value_BME68x_P = -1.0;
		last_value_BME68x_H = -1.0;
		debug_outln_error(F("BME68x read failed"));
	}

	debug_outln_info(FPSTR(DBG_TXT_SEP));
	debug_outln_verbose(FPSTR(DBG_TXT_END_READING), FPSTR(SENSORS_BME680));
}
 ...
 * Init BME68x by FEM
 *****************************************************************/
static bool initBME68x(const uint8_t addr)
{
	debug_out(String(F("Trying BME68x sensor on ")) + String(addr, HEX), DEBUG_MIN_INFO);

	bme68x.begin(addr, Wire);
	if (bme68x.checkStatus() == 0)
	{
		debug_outln_info(FPSTR(DBG_TXT_FOUND));
		bme68x.setTPH();
	...
	else
	{
		...
		{
			Serial.println("Sensor error:" + bme68x.statusString());
			debug_outln_info(FPSTR(DBG_TXT_NOT_FOUND));
			return false;
		}
		return true;
...
static void powerOnTestSensors()
{
	...
	if (cfg::sds_read)  //FEM: Settato a 0 in ext_def.h
	{
	...
	if (cfg::bme68x_read)  
	{
		debug_outln_info(F("Read BME680..."));
		if (!initBME68x(bme68x_default_i2c_address) && !initBME68x(bme68x_alternate_i2c_address))
		{
			debug_outln_error(F("Check BME68x wiring"));
			bme68x_init_failed = true;
		}
	}
...
void setup(void)
{
	...
	else if (cfg::sds_read) //FEM: Non è utilizzato nemmeno l'SDS11 (setting a 0 in ext_def.h)
	{
	...
	init_display(); //FEM: Non è previsto display, settings tutti a 0 in ext_def.h
	setupNetworkTime();
	connectWifi();
	setup_webserver();
	createLoggerConfigs();
	...
	powerOnTestSensors();
	logEnabledAPIs();
	logEnabledDisplays(); //FEM: Non è previsto display, settings tutti a 0 in ext_def.h
...
void loop(void)
{
	...
	else if (cfg::sds_read)  //FEM: Non è previsto nemmeno l'SDS11
	{
	...
	if (send_now)
	{
		...
		if (cfg::bme68x_read && (!bme68x_init_failed))
		{
			// getting temperature, humidity and pressure (optional)
			fetchSensorBME68x(result);
			data += result;
            // NOTA IMPORTANTE: 
            // i valori del BME680 sono inviati come "BME280" perché il database di Sensor Community
            // NON prevede l'uso del BME680
			sum_send_time += sendSensorCommunity(result, BME68x_API_PIN, FPSTR(SENSORS_BME280), "BME280_");
			result = emptyString;
		}
